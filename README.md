...
Web 端是纯静态 Vite 产物，不需要自建服务器：

```bash
npm run build
```

构建后的 `dist/` 可以部署到 GitHub Pages、Cloudflare Pages、Vercel 或 Netlify。当前仓库已包含 GitHub Pages workflow，推送到 `main` 后会自动部署。

Web 端受浏览器安全限制，不能像 Electron 一样内嵌官方网页。它保留 Prompt 输入、复制、收藏库和平台入口，点击平台时打开官方页面，由用户手动粘贴发送。
...
编辑 [src/config/platforms.ts](src/config/platforms.ts)：
